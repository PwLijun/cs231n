...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    W2 =self.params['W2']
    b2 = self.params['b2']
    scores_h,cache1 = affine_relu_forward(X,W1,b1)
    scores,cache2 = affine_forward(scores_h,W2,b2)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    ############################################################################
    data_loss,lscores = softmax_loss(scores,y)
    reg_loss = 0.5*self.reg*np.sum(W1*W1)+0.5*self.reg*np.sum(W2*W2)
    loss = data_loss+reg_loss
    dscorech,dW2,db2 = affine_backward(lscores,cache2)
    dx,dW1,db1 = affine_relu_backward(dscorech,cache1)
    dW1+=self.reg*W1
    dW2+=self.reg*W2
    grads['W1'] = dW1
    grads['b1'] = db1
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in xrange(self.num_layers):
        ...
        self.params['b'+str(i+1)] = np.zeros((1,layers[i+1]))
        if use_batchnorm and i<len(hidden_dims):
            self.params['gamma'+str(i+1)] = np.ones((1,layers[i+1]))
            self.params['beta'+str(i+1)] = np.zeros((1,layers[i+1]))
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    # layer, etc.                                                              #
    ############################################################################
    h, cache1, cache2, cache3, bn, outr,out = {}, {}, {}, {}, {}, {},{}
    cache4 = {}
    out[0] = X
    ...
    for i in xrange(self.num_layers-1):
        ...
        if self.use_dropout:
            if self.use_batchnorm:
                ...
                outr[i+1], cache3[i] = relu_forward(bn[i])
                out[i+1],cache4[i] = dropout_forward(outr[i+1],self.dropout_param)
            else:
                outr[i+1], cache3[i] = affine_relu_forward(out[i], W, b)
                out[i+1],cache4[i] = dropout_forward(outr[i+1],self.dropout_param)
        else:
            if self.use_batchnorm:
                gamma, beta = self.params['gamma' + str(i+1)], self.params['beta' + str(i+1)]
    ...
    for i in xrange(t):
        if self.use_dropout:
            if self.use_batchnorm:
                doutr[t-1-i] = dropout_backward(dout[t-i],cache4[t-1-i])
                dbn[t-1-i] = relu_backward(doutr[t-1-i], cache3[t-1-i])
                ...
                doutr[t-1-i] = dropout_backward(dout[t-i],cache4[t-1-i])
                dout[t-1-i], grads['W'+str(t-i)], grads['b'+str(t-i)] = affine_relu_backward(doutr[t-1-i], cache3[t-1-i])
        else:
            if self.use_batchnorm:
